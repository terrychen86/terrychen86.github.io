class Snake {
  constructor(frameSpeed, mapRowSize, mapColSize) {
    ...
    this.frameSpeed = frameSpeed || 125;
    this.mapRowSize = mapRowSize || 20;
    this.mapColSize = mapColSize || 20;
  ...
  createFood() {
    ...
    do {
      ...
      this.isPointOnSnake(
        Math.floor(randomLoc / this.mapRowNum),
        randomLoc % this.mapColNum,
      )
    );
  ...
  snakeMoves() {
    ...
    } else if (this.currentKey === 40 && this.moveOnRow !== -1) {
      this.moveOnCol = 0;
      this.moveOnRow = 1;
    }
    ...
    if (
      snakeHeadRowIdx === this.foodLoc[0] &&
      snakeHeadColIdx === this.foodLoc[1]
    ) {
      this.snakeLoc.unshift([snakeHeadRowIdx, snakeHeadColIdx]);
      this.foodEaten++;
      this.createFood();
    } else if (
      !this.isPointOnSnake(snakeHeadRowIdx, snakeHeadColIdx) &&
      !this.isPointOutOfMap(snakeHeadRowIdx, snakeHeadColIdx)
    ) {
  ...
  clearCanvas() {
    ...
    this.ctx.fillRect(
      0,
      0,
      this.mapRowSize * this.mapRowNum,
      this.mapColSize * this.mapColNum,
    );
  }
  ...
  drawPoint(row, col, color) {
    ...
    this.ctx.fillRect(
      this.mapColSize * col + 1,
      this.mapRowSize * row + 1,
      this.mapColSize - 2,
      this.mapRowSize - 2,
    );
  }
  ...
  drawStartMsg() {
    ...
    this.ctx.fillText(
      ...
      (this.mapRowSize * this.mapRowNum) / 2,
      (this.mapColSize * this.mapColNum) / 2,
    );
  }
  ...
  drawEndGameInfo() {
    ...
    this.ctx.fillText(
      "GAME OVER",
      (this.mapRowSize * this.mapRowNum) / 2,
      (this.mapColSize * this.mapColNum) / 2 + 20,
    );
    this.ctx.fillText(
      "Press Space to RESTART...",
      (this.mapRowSize * this.mapRowNum) / 2,
      (this.mapColSize * this.mapColNum) / 2 + 40,
    );
